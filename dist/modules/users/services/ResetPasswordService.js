...
let ResetPasswordService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const userToken = await this.userTokensRepository.findByToken(token); // tratamento de token, existe ou nao

    if (!userToken) {
      throw new _AppError.default('User token does not exist');
    }

    const user = await this.usersRepository.findById(userToken.user_id); // tratamento de token, existe ou nao
    ...
    const compareDate = (0, _dateFns.addHours)(tokenCreatedAt, 2); // compara o horario de criacao com a atual e expira o token caso seja maior que 2hours
